...
public class PlatformTopDivider : MonoBehaviour
{
    [Header("Configurações OG (ZigZag)")]
    public int gridColumns = 10;
    public int gridRows = 4;

    [Header("Configuração de Layers")]
    public string pieceLayer = "PlatformTopPiece"; // <-- Sugiro criares uma Layer nova tipo "PlatformTopPiece"
    public string playerLayer = "Player";          // <-- O nome exato da Layer do teu Player
    ...
    private void Start()
    {
        ...
        // --- A MAGIA DA FÍSICA: IGNORAR O PLAYER ---
        // Isto diz ao Unity para estas peças nunca baterem no Player!
        int topLayerID = LayerMask.NameToLayer(pieceLayer);
        int playerLayerID = LayerMask.NameToLayer(playerLayer);

        if (topLayerID != -1 && playerLayerID != -1)
        {
            Physics2D.IgnoreLayerCollision(topLayerID, playerLayerID, true);
        }
        else
        {
            Debug.LogWarning("Falta criar a Layer do Player ou a Layer das peças do topo nas definições do Unity!");
        }
    ...
    private void CreatePieceObjects(Dictionary<Vector2Int, List<Vector2Int>> pixelsByPiece)
    {
        ...
        foreach (var piece in pixelsByPiece)
        {
            ...
            GameObject pieceObj = new GameObject($"TopPiece_{piece.Key.x}_{piece.Key.y}");
